<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%
String contextPath = request.getContextPath(); // 반환값 : /workshop0816
%>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
	...
	<form action="">
		<h4>기본 정보</h4>
		<label for="name">이름</label><br> <input id="name" name="name"
			type="text" /><br> <label for="email">e-mail</label><br> <input
			id="email" name="email" type="email" /><br>

		<h4>성별</h4>
		<input id="male" type="radio" name="gender" value="M"><label
			for="male">남</label> <input id="female" type="radio" name="gender"
			value="F" checked><label for="female">여</label><br>

		<h4>취미</h4>
		<input id="movie" type="checkbox" name="hobby" value="movie">
		<label for="movie">영화</label> <input id="sport" type="checkbox"
			name="hobby" value="sport" checked> <label for="sport">스포츠</label>
	...
	<script>
		window.onload = function() {
			...
			document.querySelector("#btnSendPost").onclick = function() {
				...
				form.action = "<%=contextPath%>/params";
				form.method = "post";
				form.submit();
			};
